...
function TankZedModII.getWalkTypeNum(fit, zed)
    fit = fit or zed:getOutfitName()
...
function TankZedModII.setWalkType(zed)
    if TankZedModII.isTankZed(zed) then
        ...
        local walkType = zed:getVariableString("zombieWalkType")
        local num = tostring(TankZedModII.getTankZedNum(zed))

        if num and walkType and num ~= walkType then
            if num ~= walkType then
                zed:setWalkType(num)
            end
        end
