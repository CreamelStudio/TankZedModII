...
function TankZedModII.coreFunc(zed)
	...
	local isShouldSend = TankZedModII.isClosestPl(getPlayer(), zed) and isClient()
...
Commands.TankZedII.msg = function(args)
    local source = getPlayer()
    if args.id then
        local targ = getPlayerByOnlineID(args.id)
        if source ~= targ then
            if args.msg then
                TankZedModII.doSrvMsg(args.msg)
            end
        end
    end
end

Commands.TankZedII.img = function(args)
    local source = getPlayer()
    if args.id then
        local targ = getPlayerByOnlineID(args.id)
        if source ~= targ then
            if args.int then
                TankZedModII.doSrvImg(args.int)
            end
        end
    end
end

Commands.TankZedII.sfx = function(args)
    local source = getPlayer()
    if args.id then
        local targ = getPlayerByOnlineID(args.id)
        if source ~= targ then
            TankZed_ADD.doSrvSFX()
        end
    end
end
