...
function TankZedModII.isTankZed_1(zed)
    return TankZedModII.getTankZedNum(zed, TankZedModII.outfit1)
end

function TankZedModII.isTankZed_2(zed)
    return TankZedModII.getTankZedNum(zed, TankZedModII.outfit2)
end
-----------------------            ---------------------------
	...
	if  TankZedModII.isTankZed(zed) then
		...
		if fit then
			if (getCore():getDebug() and isAdmin()) then
				--print(fit)
				print(TankZedModII.getWalkType(fit))
			end

	--[[
			if TankZedModII.isTankZed_1(zed) then
				return 1
			elseif TankZedModII.isTankZed_2(zed) then
				return 2
			end  ]]
		end
	end
