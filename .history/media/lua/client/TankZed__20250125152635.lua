...
TankZedModII.skin = 'TankZedModII.Dreadnought1'
...
function TankZedModII.isTankZed_1(zed)
	...
	local num = nil
	if fit then
		num = tonumber(TankZedModII.getWalkTypeNum(fit))
	end
	if num == 1 then
		return TankZedModII.getTankZedNum(zed) == num
	end
	return false
end
...
function TankZedModII.getSandboxPage(num)
	...
	if num == 1 or num == "1"  then
		page = SandboxVars.TankZedModII_1
	elseif num == 2  or num == "2" then
		page = SandboxVars.TankZedModII_2
	end
	return page
end
...
function TankZedModII.getTankZedNum(zed)
	...
	if  TankZedModII.isTankZed(zed) then
		local fit = zed:getOutfitName()
		if fit then
			local num = tonumber(TankZedModII.getWalkTypeNum(fit))
			if num then
				return num
			end
		end
...
function TankZedModII.setTankZedII(zed, fit)
	...
	if zed then
		...
		fit = fit or 'Dreadnought'..tostring(int)
