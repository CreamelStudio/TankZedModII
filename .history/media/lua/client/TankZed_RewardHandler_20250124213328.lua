...
function TankZedModII.SpawnRewards(sq, num)
	...
	if sq then
		for _, item in ipairs(itemList) do
			local roll = TankZedModII.getRewardRoll(num)
			if roll then
...
function TankZedModII.getRewardRoll(num)
	...
	local chance = SandboxVars.TankZedModII.DropRate or 20
...
function TankZedModII.getDeathRewards(num, str)
    num = num or 1
    str = str or "LootRate"
    local tab = {
        [1] = 	{
    ...
    return tab[tonumber(tab)][tostring(str)]
end
--[[
local num = TankZedModII.getTankZedNum(zed)
TankZedModII.getDeathRewards(num, "LootRate")

local num = TankZedModII.getLootRate(zed)
TankZedModII.getDeathRewards(num, "LootRate")\
